...
$(function(){
	...
	$('#btn-salvar-pais').on('click', function(){
		$('#form-salvar-pais').submit();
		return false;
	});
});
</script>
...
<div>
	<?php
	echo $this->Form->button('Salvar', ['id' => 'btn-salvar-pais', 'class' => 'btn btn-primary']);
	echo $this->Html->link('Novo', ['action' => 'cadastrar'], ['class' => 'btn btn-success']);
	echo $this->Form->postLink('Apagar', ['action' => 'excluir'], ['data' => ['id' => $pais->id], 'class' => 'btn btn-danger']);
...
<div id="linhas-estados-cidades">
	...
	<div class="row">
		<div class="col-lg-6">
			<?php echo $this->Form->control('estados.' . $indice . '.id'); ?>
		...
		<div class="col-lg-6">
			...
			if( count($estado->cidades) > 0 ){
				foreach($estado->cidades as $indice1 => $cidades ){
					echo $this->Form->hidden('estados.' . $indice . '.cidades.' . $indice1 . '.id');
			?>
